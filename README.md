...
- **WebDAV** (☰ menu → WebDAV, any platform): links a file over the network instead of a local one, so the same inventory can be used from multiple devices. ⚠️ This is intentionally simple: every save just overwrites the remote file, and every "Pull now" overwrites your local copy — **there is no conflict resolution**. If two devices edit while both offline, whichever one saves last silently wins and the other device's changes are lost. Treat it as a shared file, not real-time collaboration. It also needs the WebDAV server to allow cross-origin requests (CORS) from wherever the app runs — most self-hosted Nextcloud/ownCloud instances support this with the right configuration; check your provider if requests fail.
